...
This chapter provides an overview of the study, covering the challenges of campus library, study’s objectives, and its significance. It defines the problem, outlines the research goals, and highlights its potential impact. The scope and limitations clarify its boundaries, while the project dictionary and notes offer essential terms and supporting details. 
...
This study aimed to develop a chatbot for CSPC library, using RAG combined with Google Gemini LLM. The goal was to address the academic community's challenges in searching and retrieving thesis literature by replacing the current, yet traditional keyword-based search with a more conversational and topic-oriented approach. This will be achieved through a website chatbot interface that is fed with this available thesis pdf data and allow users to interact by just providing their queries. This is accessible both on and off campus since it was deployed on the cloud. And the administrator can upload recent theses from the library into the system, which will go through the RAG process and be appended to the vector database.

However, this study also has some limitations.  One is the researchers will focus only on utilizing the available PDF copies of undergraduate theses that have already been published. The chatbot's accuracy will vary depending on  the quality and structure of the thesis records, as well as the clarity and relevance of the user prompts. Also, since the data is from CSPC, the users are limited to  students from the same institution and will be authenticated using their CSPC email. Furthermore, computational efficiency can be affected by the configuration and resources available in the cloud environment, limiting the chatbot's real-time processing capabilities. Finally, while the RAG approach can reduce hallucinations, users  are still advised to  validate outputs.
