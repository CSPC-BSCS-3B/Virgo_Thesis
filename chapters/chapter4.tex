...
For example, when users asked, “What research has been done on machine learning applications in healthcare?”, as shown in \ref{fig:sample_query_output} the system retrieved key sections from theses.  Notably, setting $K=50$ produced a good context and cross-thesis coverage.
...
\ref{fig:sample_query_output} shows a sample user query about existing research on machine learning applications in healthcare and the retrieved thesis key sections and summary. 
...
\ref{fig:sample_bomb_query} illustrated the capability of the RAG chatbot into responding responsibly to sensitive question user asks. 

In the case of a query is detected as violating the content guidelines, it automatically triggers guardrails protocols of the system that hinder generating responses that are harmful or inappropriate. Instead, the chatbot sends out a warning, observing ethical standards in ensuring user safety. 

This demonstrates the capability of the implemented guardrails put in place during the integration of AI in this academic environment. By prioritizing transparency and compliance, the system fosters trust and maintains the integrity of information retrieval processes.
...
Reponses that are generated by the chatbot appeared in Markdown format with citations and structured text. In cases where queries violated safety parameters, explicit warnings were shown. Deterministic settings improved consistency and user trust.
