...
Retrieved chunks were then combined with the user query in a structured context incorporating lightweight citation markers and the URL of the source document. This construction allowed for grounded and traceable responses while minimizing the chance of the hallucinations. Prompt templates guided the model to respond only based on the provided context. As \ref{fig:sample_query_output} shows, safety measures were in place to maintain input quality.
...
\ref{fig:sample_bomb_query} illustrates the capability of the RAG chatbot to respond responsibly to sensitive user queries. If a query is detected as violating the content guidelines, it automatically triggers guardrails protocols of the system that hinder generating responses that are harmful or inappropriate. Instead, the chatbot sends out a warning, observing ethical standards in ensuring user safety. This demonstrates the capability of the implemented guardrails put in place during the integration of AI in this academic environment. By prioritizing transparency and compliance, the system fosters trust and maintains the integrity of information retrieval processes.
...
Generated responses appeared as Markdown with citations and structured text. In cases where queries violated safety parameters, explicit warnings were shown. Deterministic settings improved consistency and user trust.
...
\ref{tab:user_agreement_quality} summarizes the results of the user-centered assessment of the CSPC Library RAG chatbot using 5-point Likert-scale survey questions that allow respondents to evaluate and express their level of agreement about the response quality and performance of the chatbot.
