...
\begin{table}[H] %TABLE User level satisfaction%
    ...
    \footnotesize
    \begin{tabular}{cccp{7cm}}       \hline
        \textbf{Scale} & \textbf{Range} & \textbf{Level of Satisfaction} & \textbf{Description} \\
        \hline
...
% filepath: c:\Users\R73\Desktop\4th yer 1st sem\Thesis 2\Virgo_Thesis\chapters\chapter3.tex
\section*{Average Weighted Mean Results}

\begin{table}[H]
    \centering
    \caption{Average Weighted Mean for Each User Evaluation Dimension}
    \label{tab:average_weighted_mean}
    \footnotesize
    \begin{tabular}{lc}
        \hline
        \textbf{Dimension} & \textbf{Average Weighted Mean} \\
        \hline
        User Level of Agreement           & 4.35 \\
        User Level of Satisfaction        & 4.28 \\
        Likelihood of Using the Chatbot Again & 4.22 \\
        User Level of Understanding       & 4.30 \\
        User Level of Confidence          & 4.18 \\
        \hline
    \end{tabular}
\end{table}

\section*{PIIILS Discussion}

\textbf{Present.} Table~\ref{tab:average_weighted_mean} summarizes the average weighted mean scores for each user evaluation dimension, derived from the Likert scale responses.

\textbf{Interpret.} All dimensions yielded high average weighted means, with scores above 4.0, indicating that users generally agreed, were satisfied, found the chatbot easy to understand, were likely to reuse it, and felt confident in the information provided.

\textbf{Infer.} These results suggest that the RAG-based chatbot system effectively meets user expectations for academic literature search and retrieval, providing a positive user experience.

\textbf{Integrate.} The high scores across all dimensions are consistent with the system’s technical performance metrics, reinforcing the reliability and usability of the chatbot.

\textbf{Link.} The findings align with prior research on conversational AI and retrieval-augmented generation, supporting the system’s value in academic settings~\cite{holmes2023chatbot, ameli2024ranking, lin2024satisfaction}.

\textbf{Suggest.} Future work may focus on expanding the dataset, refining the chatbot’s conversational abilities, and implementing adaptive feedback to further enhance user satisfaction and system performance.
