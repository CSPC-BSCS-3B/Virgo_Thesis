...
\ref{tab:likert_scale} shows that this study employed a 5-point Likert Scale to determine "User level of Agreement" based on the user’s experience with the system’s response quality and performance. In the table, the first column showed the scale, and the second its range, while the scale that the system level of agreement fell under was shown in the third column, and its corresponding description in the fourth column. The user’s response was computed using a weighted mean, and then it was determined in which range it fell.  
