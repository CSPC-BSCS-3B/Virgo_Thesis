...
The system’s technical performance is evaluated using the RAGAS framework, focusing on context precision, recall, relevance, and faithfulness to measure how well relevant documents were retrieved and responses generated~\cite{holmes2023chatbot, ameli2024ranking, lin2024satisfaction}.
...
\begin{table}[H] %TABLE 3%
    ...
    \end{tabular}
\end{table}
\ref{tab:likert_scale} presents the Likert Scale used to measure the user Level of Agreement with the chatbot responses. The scale helps summarize how users felt about the quality and performance of the system answers during their interaction. The table lists the scale values, the numeric ranges, the agreement labels, and a short description for each level. Using this structure made it easier to compute a weighted mean across all responses and to place the overall result within a clear range. In simple terms, the higher the scale value, the stronger the agreement with the statement about the chatbot responses.

Scale five is labeled Strongly Agree. It means the participant completely agrees with the statement and finds the chatbot output excellent in quality and performance. It reflects strong support for the system reply and a high level of confidence in its usefulness. Scale four is labeled Agree. It means the participant shows positive agreement but not at the highest possible level. The user generally finds the answer good and helpful and is satisfied with the way the system addressed the query.

Scale three is labeled Neutral. It shows that the participant does not lean toward agreement or disagreement. The user may feel unsure or undecided about the statement. The response might be acceptable but not convincing enough to form a clear opinion. Scale two is labeled Disagree. It signals that the participant does not support the statement. The answer may miss important points, lack clarity, or not fully match the user expectation, but the disagreement is not at the strongest level.

Scale one is labeled Strongly Disagree. It conveys a clear and firm disagreement with the statement. The participant finds the response poor in quality and performance and does not trust its usefulness. When many users select this level, it suggests that the system output does not meet their needs and that improvements are required.

By organizing feedback with this scale, the study can convert individual opinions into measurable data that are easy to compare and analyze. The weighted mean places the overall experience within a numeric range and a label that describe the strength of agreement. This helps identify what users value most, such as accuracy, clarity, or relevance, and it highlights where the chatbot should improve, such as context handling or evidence support. The approach makes the evaluation fair and consistent and supports transparent reporting of results.
